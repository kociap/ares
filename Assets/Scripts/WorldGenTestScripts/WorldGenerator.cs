...
public enum TileType { EMPTY, ROOM, BORDER, CORIDOR_Y, CORIDOR_X, ROOM_INIT }
...
public class WorldGenerator : MonoBehaviour
{
    ...
    public void ChangePixelColor(Vector2 point, TileType state)
    {
    ...
    // After room is created adds new room connection to connection lists
    ...
    {
        return horizontalConnecting ? connectionsX[Random.Range(0,connectionsX.Count)] :
    ...
    void ArrangeRoomBorders() 
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (IsBorderTile(i, j))
                {
                    ChangePixelColor(new Vector2(i, j), TileType.BORDER);
                }
            }
        }
    }
    ...
    void RemoveDeadEnds()
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                {
                    ChangePixelColor(new Vector2((float)i, (float)j), TileType.EMPTY);
                }
            }
        }
    }
